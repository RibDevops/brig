...
def instrucao_nova(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('instrucao_lista')  # Redirecione para a lista após criar
    else:
        form = InstrucaoForm()
...
def instrucao_editar(request, id):
    context ={}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('instrucao_lista')
    else:
...
def instrucao_delete(request, id):
    context ={}
    instrucao_ob = get_object_or_404(Instrucao, id=id)
    if request.method == 'POST':
        instrucao_ob.delete()
